...
def main(annotations_path, mimic_path, output_path):
    """
    Main function to merge annotations with MIMIC-III data and append note text.

    Args:
        annotations_path (str): CSV path containing insomnia annotations.
        mimic_path (str): Directory path containing MIMIC-III CSV files.
        output_path (str): Output CSV file with appended text.
    """
    # Data loading
    df_ann = pd.read_csv(annotations_path)
    df_ann["note_id"] = df_ann["note_id"].astype(int)

    df_mimic_note = pd.read_csv(os.path.join(mimic_path, 'NOTEEVENTS.csv.gz'))
    df_mimic_note.index = df_mimic_note["ROW_ID"].values

    df_mimic_presc = pd.read_csv(os.path.join(mimic_path, 'PRESCRIPTIONS.csv.gz'))
    df_mimic_patient = pd.read_csv(os.path.join(mimic_path, 'PATIENTS.csv.gz'))
    ...
    # Save data
    df_data[list(df_ann.columns) + ["text"]].to_csv(output_path, sep=',', index=False)


if __name__ == "__main__":
    # Initialize parser
    parser = argparse.ArgumentParser(description="Append MIMIC-III note texts to annotation CSV file.")
    parser.add_argument(
        "-a",
        "--annotations_path",
        type=str,
        required=True,
        help="Path to the CSV file with insomnia annotations"
    )
    parser.add_argument(
        "-m",
        "--mimic_path",
